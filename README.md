...
Smart Theatre is a fog-edge monitoring demo that generates sensor readings, sends them through SQS and Lambda, stores them in SQLite, and visualizes them in a Flask dashboard.

## Flow

1. `Fog_node.py` generates sensor readings every 5 seconds.
2. Sensor payloads are sent to Amazon SQS.
3. AWS Lambda processes SQS messages and writes to `smart_theatre.db` in `/tmp`.
4. Flask reads from local SQLite and serves the dashboard.
5. `dashboard.html` refreshes every 5 seconds.
...
- The dashboard uses SQLite data written by `Fog_node.py`.
- `.env`, `smart_theatre.db`, and `__pycache__` files should stay out of Git.
- `.env`, `smart_theatre.db`, and `__pycache__` files should stay out of Git.
...
1. **Fog_node.py** generates sensor values every 5 seconds.
2. **Sensor payload sent to SQS** (`SmartTheatreQueue`).
3. **AWS Lambda** (`ProcessTheatreSensors`) triggered by SQS event.
4. **Lambda stores to SQLite** (`smart_theatre.db`) in `/tmp` (local ephemeral storage).
5. **Lambda logs to CloudWatch** (monitoring + audit trail).
...
flowchart TD
    subgraph FOG["🔵 FOG LAYER (Local)"]
        GEN["Fog_node.py<br/>━━━━━━━━━━━━━━━━<br/>• Python 3.14<br/>• boto3 SQS client<br/>• Generate 5 sensors<br/>• Timestamp ISO format"]
        SENSORS["Sensors Generated<br/>━━━━━━━━━━━━━━━<br/>• Temperature °C<br/>• Motion %<br/>• Light lux<br/>• Noise dB<br/>• Smoke %"]
        GEN --> SENSORS
    end
...
| # | Source | Action | Output | Tools |
|---|--------|--------|--------|-------|
| 1 | FOG | Generate 5 sensor readings | JSON `{sensor, value, status, unit, timestamp}` | `Fog_node.py`, `boto3` |
| 2 | FOG → Cloud | Send to SQS queue | Message in queue | Amazon SQS |
| 3 | Cloud | Lambda triggered by SQS | Lambda invocation | AWS Lambda |
| 4 | Lambda | Parse & validate message | Extracted sensor data | Python 3.12 |
| 5 | Lambda | Store to SQLite | INSERT record | SQLite in `/tmp/` |
...
- Flask no longer consumes SQS (to avoid competing with Lambda).
- Dashboard data is sourced from local SQLite entries written by `Fog_node.py`.
- Lambda logs/metrics confirm SQS → Lambda processing in cloud.
- If CloudWatch log stream list looks stale, use **Live tail** or set recent time range and refresh.
>>>>>>> 4abb537 (Refine Smart Theatre dashboard and generator)
